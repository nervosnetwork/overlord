...
async fn test_process(case: WalTestCase) {
    let (overlord_tx, overlord_rx) = unbounded();
    let (_raw_tx, raw_rx) = fut_unbounded();
    let (_mock_tx, mock_rx) = fut_unbounded();
    ...
    runtime::spawn(async move { overlord.run(raw_rx, Event::new(mock_rx), tmp_rx).await });
